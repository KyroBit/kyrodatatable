 ...
 */
export function DataTable({ api, presets, filterColumns, searchPlaceholder, searchWidth, createLabel = 'Create', onCreate, onExport, exportFormats = [{ id: 'csv', label: 'CSV' }], onImport, exportTooltip = 'Export current results', importTooltip = 'Import from CSV', actions, selectable, onRowClick, rowClass, emptyMessage, stickyHeader, rowsPerPageOptions, }) {
    ...
    return (_jsxs(DataTableRoot, { api: api, children: [(presets || onCreate) && (_jsxs(Stack, { direction: "row", sx: { alignItems: 'center', justifyContent: 'space-between', gap: 1.5 }, children: [_jsx(Stack, { direction: "row", spacing: 1, sx: { alignItems: 'center', minWidth: 0 }, children: presets && (_jsxs(_Fragment, { children: [_jsxs(ToggleButtonGroup, { exclusive: true, value: activePresetId, onChange: (_, nextId) => {
                                    ...
                                    }, children: [_jsx(ToggleButton, { value: "all", "data-label": "All", children: "All" }), presets.all.map((p) => (_jsx(ToggleButton, { value: p.id, "data-label": p.name, children: p.name }, p.id)))] }), _jsx(Tooltip, { title: "Manage views", children: _jsx(IconButton, { onClick: () => setManageOpen(true), "aria-label": "Manage views", sx: (theme) => ({
                                            width: 45,
                                            height: 45,
                                            borderRadius: '8px',
