...
export function DataTableBody({ onRowClick, emptyMessage = 'No records found.', selectable = false, stickyHeader = true, }) {
    ...
    return (_jsx(TableContainer, { sx: { flex: '1 1 auto', minHeight: 0 }, children: _jsxs(Table, { size: "small", stickyHeader: stickyHeader, children: [_jsx(TableHead, { children: _jsxs(TableRow, { children: [selectable && (_jsx(TableCell, { padding: "checkbox", children: _jsx(Checkbox, { size: "small", checked: api.allVisibleSelected, indeterminate: api.someVisibleSelected && !api.allVisibleSelected, onChange: () => api.toggleSelectAll(), slotProps: { input: { 'aria-label': 'Select all rows' } } }) })), api.columns.map((col) => (_jsx(TableCell, { align: col.align, sortDirection: api.sort?.field === col.field ? api.sort.direction : false, onClick: col.sortable === false ? undefined : () => api.toggleSort(col.field), sx: col.sortable === false ? undefined : {
                                    cursor: 'pointer',
                                    userSelect: 'none',
