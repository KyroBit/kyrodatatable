...
export function ManageViewsDialog({ open, onClose, presets, filterColumns, onApply }) {
    ...
    return (_jsx(Dialog, { open: open, onClose: close, slotProps: { paper: { sx: { width: 480, maxWidth: '92vw', borderRadius: '16px' } } }, children: _jsxs(Box, { sx: { px: 3, pt: 2, pb: 2.5 }, children: [_jsxs(Stack, { direction: "row", sx: { alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }, children: [_jsxs(Stack, { direction: "row", spacing: 1, sx: { alignItems: 'center' }, children: [editing && (_jsx(IconButton, { onClick: () => setEditingId(null), "aria-label": "Back to views", sx: { width: 28, height: 28, ml: -0.5, color: 'text.primary' }, children: _jsx(ArrowBackRoundedIcon, { sx: { fontSize: 17 } }) })), _jsx(Typography, { sx: { fontSize: '18px', fontWeight: 600, color: 'text.primary' }, children: editing ? 'Edit view' : 'Views' })] }), _jsx(IconButton, { onClick: close, "aria-label": "Close views", sx: (theme) => ({
                                width: 28,
                                height: 28,
                                bgcolor: alpha(theme.palette.text.primary, 0.04),
                                color: theme.palette.text.primary,
                                ...
                                e.preventDefault();
                                commitEdit();
                            } } }), filterColumns.map((column) => (_jsxs(Box, { children: [_jsx(Typography, { sx: { fontSize: '13.5px', fontWeight: 600, color: 'text.primary', mt: 2, mb: 1 }, children: column.label }), _jsx(FilterField, { column: column, value: editFilters[column.field], onChange: (next) => setEditFilters({ ...editFilters, [column.field]: next }) })] }, column.field))), _jsxs(Stack, { direction: "row", spacing: 1, sx: { justifyContent: 'flex-end', mt: 3 }, children: [_jsx(Button, { variant: "outlined", color: "inherit", onClick: () => setEditingId(null), sx: { height: 36, minHeight: 36, px: '16px', fontSize: '13px', borderRadius: '8px' }, children: "Cancel" }), _jsx(Button, { variant: "contained", disabled: !editName.trim(), onClick: commitEdit, sx: { height: 36, minHeight: 36, px: '20px', fontSize: '13px' }, children: "Save changes" })] })] })) : (_jsxs(_Fragment, { children: [_jsx(Stack, { spacing: 0.25, sx: { mt: 1.5, maxHeight: 380, overflowY: 'auto' }, children: _jsx(DndContext, { sensors: sensors, collisionDetection: closestCenter, modifiers: [restrictToVerticalAxis, restrictToParentElement], onDragEnd: handleDragEnd, children: _jsx(SortableContext, { items: presets.all.map((p) => p.id), strategy: verticalListSortingStrategy, children: presets.all.map((p) => (_jsxs(SortableViewRow, { id: p.id, onClick: () => { onApply(p.filters, p.id); close(); }, children: [_jsxs(Box, { sx: { flex: 1, minWidth: 0 }, children: [_jsx(Typography, { noWrap: true, sx: { fontSize: '14px', fontWeight: 600, color: 'text.primary' }, children: p.name }), _jsx(Typography, { noWrap: true, sx: { fontSize: '12.5px', color: 'text.secondary' }, children: summarizeFilterValues(p.filters, filterColumns) })] }), _jsxs(Stack, { direction: "row", spacing: 0.25, onClick: (e) => e.stopPropagation(), children: [_jsx(Tooltip, { title: "Edit", children: _jsx(IconButton, { size: "small", "aria-label": `Edit ${p.name}`, onClick: () => startEdit(p.id), sx: { color: 'text.secondary' }, children: _jsx(EditIcon, { sx: { fontSize: 15 } }) }) }), _jsx(Tooltip, { title: "Delete", children: _jsx(IconButton, { size: "small", "aria-label": `Delete ${p.name}`, onClick: () => presets.remove(p.id), sx: { color: 'error.main' }, children: _jsx(TrashIcon, { sx: { fontSize: 15 } }) }) })] })] }, p.id))) }) }) }), _jsx(Stack, { direction: "row", sx: { justifyContent: 'flex-end', mt: 2 }, children: _jsx(Button, { variant: "contained", onClick: close, sx: { height: 36, minHeight: 36, px: '24px', fontSize: '13px' }, children: "Done" }) })] }))] }) }));
}
